...
export function VideoPlayer({
  ...
  // Audio-only móvil para HLS (no aplica a YouTube)
  useEffect(() => {
  ...
  // Media Session (no afecta a YouTube, pero tampoco molesta)
  useEffect(() => {
  ...
  // Visibilidad / background (solo HLS nativo; YouTube maneja lo suyo)
  useEffect(() => {
    if (isYouTube) return;
    const onVis = () => {
      const v = videoRef.current;
  ...
  // Listener para restaurar HLS después de screen sharing
  useEffect(() => {
    const video = videoRef.current;
    if (!video || isYouTube) return;

    const handleRestoreHls = (event: CustomEvent) => {
      const { streamUrl: restoreUrl } = event.detail;
      if (!restoreUrl) return;

      // Limpiar cualquier HLS existente
      if (hlsRef.current) {
        hlsRef.current.destroy();
        hlsRef.current = null;
      }

      // Restaurar el stream HLS
      setTimeout(() => {
        setupHLSPlayer(restoreUrl);
      }, 200);
    };

    video.addEventListener('restore-hls-stream', handleRestoreHls as EventListener);
    
    return () => {
      video.removeEventListener('restore-hls-stream', handleRestoreHls as EventListener);
    };
  }, [isYouTube]);
  ...
  // Inicialización
  useEffect(() => {
    // Si es YouTube, no arrancamos FFmpeg ni HLS
    if (isYouTube) {
  ...
  return (
    <div className="relative mb-4 md:mb-6">
      <div className="video-player bg-black rounded-md md:rounded-lg overflow-hidden shadow-lg md:shadow-2xl aspect-video">
        ...
        {/* Controles de transmisión - Solo para contenido HLS, no YouTube */}
        {!isYouTube && (
        ...
        {/* Overlay de ayuda para background (solo HLS) */}
